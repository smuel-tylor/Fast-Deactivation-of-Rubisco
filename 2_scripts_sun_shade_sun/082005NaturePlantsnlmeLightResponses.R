...
#using the same data as 082005summarize.R
...
AQ<-AQ[order(AQ$plant), ]#done to get the plots etc. in a nice order

AQ.nlsList <- nlsList(
												A ~ ((phi * Qin + Asat - sqrt((phi * Qin+Asat)^2 - (4 * theta * phi * Qin * Asat))) / (2 * theta)) - Rd | plant, 
												start = c(phi = 0.05, Asat = 30, theta = 0.75, Rd = 1.9),
												#had to do the below because there was some issue with one of the other columns
...
AQ.nls <- nls(
							A ~ ((phi * Qin + Asat - sqrt((phi * Qin+Asat)^2 - (4 * theta * phi * Qin * Asat))) / (2 * theta)) - Rd, 
							start = c(phi = 0.05, Asat = 30, theta = 0.75, Rd = 1.9), 
							data = AQ, na.action = na.exclude
							)
summary(AQ.nls)
...
#So, the model requires both fixed and rondome effects for all parameters
...
nd.AQ <- data.frame(Qin = rep(q.smth, length(p.smth)), 
											geno = rep(g.smth, each = length(q.smth)), 
											plant = rep(p.smth, each = length(q.smth))
											)
#nd.AQ$plant <- factor(nd.AQ$plant, levels = levels(AQ$plant))
#nd.AQ$geno <- factor(nd.AQ$geno, levels = levels(AQ$geno))

pred.AQ.nlme2 <- data.frame(A.geno = predict(AQ.nlme2, newdata = nd.AQ, level = 0), 
...
pdf("082005NaturePlantsnlmeLightResponses_nlme2.pdf", w = 6, h = 9)							
...
fixphi <- c(fixef(AQ.nlme2)[1], fixef(AQ.nlme2)[1] + fixef(AQ.nlme2)[c(2:4)])
cbind(fixphi, fixphi + cis[c(1:4)] %*% cbind(-1, 1))
fixAsat <- c(fixef(AQ.nlme2)[5], fixef(AQ.nlme2)[5] + fixef(AQ.nlme2)[c(6:8)])
cbind(fixAsat, fixAsat + cis[c(5:8)] %*% cbind(-1, 1))
fixtheta <- c(fixef(AQ.nlme2)[9], fixef(AQ.nlme2)[9] + fixef(AQ.nlme2)[c(10:12)])
cbind(fixtheta, fixtheta + cis[c(9:12)] %*% cbind(-1, 1))
fixRd <- c(fixef(AQ.nlme2)[13], fixef(AQ.nlme2)[13] + fixef(AQ.nlme2)[c(14:16)])
cbind(fixRd, fixRd + cis[c(13:16)] %*% cbind(-1, 1))

AQ.fixed <- rbind(cbind(fixphi, fixphi + cis[c(1:4)] %*% cbind(-1, 1)),
								cbind(fixAsat, fixAsat + cis[c(5:8)] %*% cbind(-1, 1)),
								cbind(fixtheta, fixtheta + cis[c(9:12)] %*% cbind(-1, 1)),
								cbind(fixRd, fixRd + cis[c(13:16)] %*% cbind(-1, 1))
								)
...
save(AQ.fixed, AQ.nlme2, AQ, file = "082005NaturePlantsnlmeLightResponses.Rdata")
