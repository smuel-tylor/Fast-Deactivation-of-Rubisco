...
#nlme....R further process (or in the case of A/PPFD substitute for)
# these analyses prior to use in diurnal models (diurnal.R)
...
source(here("4_source_scripts_AQ_Aci_Vcmaxt_diurnal/AstarVcmax.R"))

#specific to the analysis presented
source(here("4_source_scripts_AQ_Aci_Vcmaxt_diurnal/functions_summarize.R"))

#Load and clean dataset (cp) and separate data objects for AC, AQ and inds
source(here("1_scripts_response_curves/cleanup.R"))

#a pdf that will hold all of the graphical output from analyses below
pdf(here("output/summarize.pdf"), h = 11, w = 8, paper = "a4")
...
#first model
#for start values, see doAQfit in: functions_summarize.R
...
#the below go forwards for additional analysis in nlmeVcmax.R

save(inds.list_GammastarKcogm_fixed_paired,
			inds.list_GammastarKcoInfgm_fixed_paired,
			file = here("output/Astar.Rdata")
			)

save.image(here("output/summarize.Rdata"))
